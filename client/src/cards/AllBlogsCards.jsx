    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[30px] border border-[#dbe6b8] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,247,232,0.96))] transition duration-300 hover:-translate-y-1 hover:border-[#c8d79f]">
      <div className="relative h-56 overflow-hidden">
        <SmartImage
          ...
          alt={blog.title}
          fallbackLabel={
            blog.author ? `${blog.author}'s story` : "Story visual"
          }
      ...
      <div className="flex flex-1 flex-col p-3 sm:min-h-[18rem] h-auto md:min-h-[15rem]">
        ...
        <h3 className="font-display text-2xl leading-tight text-slate-900">
          ...
          >
            {blog.title.length > 72
              ? `${blog.title.substring(0, 72)}...`
              : blog.title}
          </Link>
        </h3>
        ...
        {/* <Link
          to={blogPath}
          className="mt-5 rounded-2xl bg-[#a8cb73] px-4 py-3 no-underline text-center inline-flex self-end text-sm font-semibold text-[#24311f] transition hover:scale-[1.01] hover:bg-[#9fc46b]"
        >
          Read
        </Link> */}
      </div>
